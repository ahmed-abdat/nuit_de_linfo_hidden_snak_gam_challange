...
function autoBind(instance: object) {
  ...
  Object.getOwnPropertyNames(proto).forEach((key) => {
    if (key !== "constructor" && typeof inst[key] === "function") {
      inst[key] = (inst[key] as (...args: unknown[]) => unknown).bind(instance);
    }
  });
}
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          // CONTAIN mode: show full image without cropping (max instead of min)
          vec2 ratio = vec2(
          ...
          // Discard pixels outside image bounds (letterbox areas)
          if (uv.x < 0.0 || uv.x > 1.0 || uv.y < 0.0 || uv.y > 1.0) discard;

          vec4 color = texture2D(tMap, uv);

          // Early discard for fully transparent pixels (performance optimization)
          ...
          // Preserve image transparency while applying border radius mask
...
class App {
  ...
  onWheel(e: WheelEvent) {
    // deltaY is standard; wheelDelta and detail are legacy fallbacks
