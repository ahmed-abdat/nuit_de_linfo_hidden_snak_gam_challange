...
const DecayCard: React.FC<DecayCardProps> = ({
  ...
  // Initialize refs with null, set in useEffect to avoid SSR/render issues
  const cursor = useRef<{ x: number; y: number } | null>(null);
  const cachedCursor = useRef<{ x: number; y: number } | null>(null);
  const winsize = useRef<{ width: number; height: number } | null>(null);

  useEffect(() => {
    // Initialize refs on mount (client-side only)
    cursor.current = {
      x: window.innerWidth / 2,
      y: window.innerHeight / 2
    };
    cachedCursor.current = {
      x: window.innerWidth / 2,
      y: window.innerHeight / 2
    };
    winsize.current = {
    ...
    const render = () => {
      // Early return if refs not initialized
      if (!cursor.current || !cachedCursor.current || !winsize.current) {
